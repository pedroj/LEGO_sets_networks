...
# colors <- readr::read_csv(here::here("data/rebrickable/colors.csv"))
# inventories <- readr::read_csv("data/rebrickable/inventories.csv")
# inventory_parts <- readr::read_csv("data/rebrickable/inventory_parts.csv")
# inventory_sets <- readr::read_csv("data/rebrickable/inventory_sets.csv")
# part_categories <- readr::read_csv("data/rebrickable/part_categories.csv")
# parts <- readr::read_csv("data/rebrickable/parts.csv")
# sets <- readr::read_csv("data/rebrickable/sets.csv")
# themes <- readr::read_csv("data/rebrickable/themes.csv")
# elements <- readr::read_csv("data/rebrickable/elements.csv")
# inventory_minifigs <- readr::read_csv("data/rebrickable/inventory_minifigs.csv")
# minifigs <- readr::read_csv("data/rebrickable/minifigs.csv")
# part_relationships <- readr::read_csv("data/rebrickable/part_relationships.csv")
...
write_csv(themes, "data/rebrickable/themes.csv")
colors <- dwld(urlcolors)
write_csv(colors, "data/rebrickable/colors.csv")
inventories <- dwld(urlinventories)
write_csv(inventories, "data/rebrickable/inventories.csv")
inventory_parts <- dwld(urlinventory_parts)
write_csv(inventory_parts, "data/rebrickable/inventory_parts.csv")
inventory_sets <- dwld(urlinventory_sets)
write_csv(themes, "data/rebrickable/inventory_sets.csv")
part_categories <- dwld(urlpart_categories)
write_csv(themes, "data/rebrickable/part_categories.csv")
parts <- dwld(urlparts)
write_csv(themes, "data/rebrickable/parts.csv")
sets <- dwld(urlsets)
write_csv(themes, "data/rebrickable/sets.csv")
themes <- dwld(urlthemes)
write_csv(themes, "data/rebrickable/themes.csv")
elements <- dwld(urlelements)
write_csv(themes, "data/rebrickable/elements.csv")
inventory_minifigs <- dwld(urlinventory_minifigs)
write_csv(themes, "data/rebrickable/inventory_minifigs.csv")
minifigs <- dwld(urlminifigs)
write_csv(themes, "data/rebrickable/minifigs.csv")
part_relationships <- dwld(urlpart_relationships)
write_csv(themes, "data/rebrickable/part_relationships.csv")
...
lego_sets <- dplyr::left_join(sets, themes, by=c("theme_id" = "id"))
lego_sets <- dplyr::rename(lego_sets, "set_name"=name.x, "theme_name"=name.y,
                    "set_img_url"= img_url)
write_csv(lego_sets, "data/rebrickable/lego_sets.csv")
...
lego_parts <- dplyr::left_join(inventory_parts, inventories,
  by=c("inventory_id" = "id")) %>%
  dplyr::left_join(parts, by="part_num") %>%
  dplyr::left_join(part_categories, by=c("part_cat_id"="id")) %>%
  dplyr::rename("part_name"=name.x, "part_category"=name.y) %>%
  dplyr::left_join(colors, by=c("color_id"="id")) %>%
  dplyr::rename(color_name="name") %>%
  dplyr::left_join(sets, by="set_num") %>%
  dplyr::left_join(themes, by=c("theme_id" = "id")) %>%
  dplyr::rename("set_name"=name.x, "theme_name"=name.y)

lego_parts <- dplyr::rename(lego_parts, "set_img_url"=img_url.y, 
                            "num_parts"= num_parts.y, "part_img_url"= img_url.x)

lego_parts<- lego_parts %>% dplyr::select(theme_name, 
                      theme_id, 
                      set_img_url,
                      set_num, 
                      set_name, 
                      ...
                      is_spare,
                      version)
write_csv(lego_parts, "data/rebrickable/set_parts.csv")

```

## Reationships between no. parts in set and no. part types
```{r n_parts-vs-n_part-types }
# List the theme_names
set_cnt_by_theme<- lego_sets  %>% 
     dplyr::count(theme_name, sort = TRUE, name = "set_count")

head(set_cnt_by_theme[,1],50L)
...
gg <- ggplot(by_set_nparts_ntypes, aes(x = nparts, y = ntypes)) +
    geom_point(size= 3, color= "blue", alpha=0.35)                           +
...
Starwars<- lego_parts %>%
         ...
         dplyr::filter(theme_name == "Star Wars")%>% 
         dplyr::filter(set_num != "SWMP-1") # Leaves out set #SWMP-1, Promo Mosaic
...
```{r diversity_set, echo=TRUE}
# Filtering some sets
tt<- Starwars %>% dplyr::filter(set_num %in% c(tail(stw_H$set_num,80)))  %>% 
        dplyr::group_by(set_num) %>%  # Group to compute H per set
        dplyr::summarize(H= vegan::diversity(table(quantity), 
                 index = "shannon", MARGIN = 1, base = exp(1)))

# Just a set:
tt<- Starwars %>% dplyr::filter(set_num == "66535-1")  %>% 
        dplyr::summarize(H= vegan::diversity(table(quantity), 
                 index = "shannon", MARGIN = 1, base = exp(1)))
...
# Groups of models
stw_H<- Starwars %>%
    dplyr::group_by(set_num, set_name) %>%
    dplyr::summarize(H= vegan::diversity(table(quantity), 
                 index = "shannon", MARGIN = 1, base = exp(1)))  %>% 
    dplyr::arrange(desc(H))
print.data.frame(stw_H, digits = 10)
...
stw_div_indexes <- stw_H %>%
  dplyr::left_join(stw_Simp,   by = join_by(set_name)) %>%
  dplyr::left_join(stw_invSimp,by = join_by(set_name)) %>%
  dplyr::left_join(stw_unbSimp,by = join_by(set_name)) %>%
  dplyr::left_join(stw_S,      by = join_by(set_name))  %>% 
  dplyr::mutate(stw_J= H/log(S))  # Adding J index.

#-----------------------------------

stw_div_indexes<- stw_div_indexes %>%
                  dplyr::left_join(sets, by = join_by(set_num))
stw_div_indexes<- stw_div_indexes %>%
                  dplyr::select(set_num, set_name, H, simp, invSimp, 
                                unbias.simp, S, stw_J, year, theme_id, 
                                num_parts, img_url) %>% # Filtering out H'=0 records.
        dplyr::filter(H != 1)

# Plots of H' vs no. parts for sets.
# 
plot(stw_div_indexes$num_parts,stw_div_indexes$H)
gg <- ggplot(stw_div_indexes, aes(x = num_parts, y = H)) +
        geom_point(size= 3, color= "blue", alpha=0.35)   +
        scale_x_log10() + scale_y_log10() +
        theme_bw()
gg
...
# helper: Hill numbers from a vector of abundances
hill_numbers <- function(x) {
  x <- x[x > 0]
  p <- x / sum(x)
  # q = 0: richness
  D0 <- length(x)
...
hill_df <- Starwars %>%
#  filter(set_num %in% tt$set_num) %>%        # To select subsets.
  dplyr::group_by(set_num, set_name) %>%
  dplyr::summarise(
    vals = list(hill_numbers(table(quantity))),
    .groups = "drop"
  ) %>%
  dplyr::mutate(
    D0 = map_dbl(vals, 1),
...
# This gives one row per set with Hill D0 (richness), D1 (exp‑Shannon), 
# and D2 (inverse Simpson).
...
# Plot (bars by set, facets by q).
# 
ggplot(hill_long, aes(x = reorder(set_num, D), y = D, fill = q)) +
  ...
  labs(
    x = "Set number",
    y = "Hill number D",
    title = "Hill numbers (q = 0, 1, 2) for 80 Star Wars sets"
  ) +
  theme_minimal()
# This shows how effective diversity declines from q=0 (richness) to q=2 (dominance‑weighted),
# revealing which LEGO Star Wars sets have both high part richness and evenness.

#----------------------------------------------------------------------------------------------
# With package hillR.
# same as: exp(vegan::diversity(x = dummy$abun, index = 'shannon'))
#  filter(set_num %in% tt$set_num) %>%        # To select subsets.
starwars_hill_df <- lego_parts %>%  
    dplyr::filter(theme_name== "Star Wars")  %>% 
    dplyr::group_by(set_num) %>% 
    dplyr::select(set_num, part_num, num_parts, quantity)%>% 
    dplyr::filter(set_num != "SWMP-1") # Filtering out set SWMP-1, Promo Mosaic

# Build community matrix and compute Hill numbers
# starwars_hill_df: set_num, part_num, num_parts, quantity
# 1. Ensure one row per set_num × part_num (sum if duplicated)
lego_comm <- starwars_hill_df %>%
  group_by(set_num, part_num) %>%
  summarise(quantity = sum(quantity), .groups = "drop") %>%
# 2. Wide format: rows = sets, columns = parts, values = quantities
  pivot_wider(
    names_from  = part_num,
    values_from = quantity,
    values_fill = 0
  ) %>%
  column_to_rownames("set_num") %>%
  as.matrix()

# 3. Hill numbers per set for q = 0, 1, 2
hill_q0 <- hill_taxa(lego_comm, q = 0)  # richness
hill_q1 <- hill_taxa(lego_comm, q = 1)  # exp(Shannon)
hill_q2 <- hill_taxa(lego_comm, q = 2)  # inv Simpson

hill_by_set <- tibble(
  set_num = rownames(lego_comm),
  q0 = as.numeric(hill_q0),
  q1 = as.numeric(hill_q1),
  q2 = as.numeric(hill_q2) )  %>% 
    dplyr::filter(q0 != 1)  %>% 
    dplyr::filter(set_num != "SWMP-1")
    
hill_by_set <- hill_by_set %>% 
    dplyr::left_join(lego_sets, hill_by_set, by="set_num") %>% 
    dplyr::select("set_num", "set_name", "year", "num_parts", 
                  "q0", "q1", "q2", "set_img_url")  %>% 
    dplyr::arrange(desc(num_parts))

# Plots of H' vs no. parts for sets.
# 
plot(hill_by_set$num_parts,log(hill_by_set$q1))
gg <- ggplot(hill_by_set, aes(x = num_parts, y = q1)) +
    geom_point(size= 3, color= "blue", alpha = 0.35) +
    scale_x_log10() + scale_y_log10() +
    theme_bw()
gg
...
subset_themes_n10<- lego_sets %>% # Subsetting main themes (those with ≥ 10 sets).
     ...
     dplyr::select(theme_name, theme_id, set_num, set_name, year, num_parts) %>% 
...
#    dplyr::filter(!grepl('SWMP-1', set_num))  %>% # StarWars Promo Mosaic set
...
subset_parts_themes_n10<- lego_parts %>% 
     ...
     dplyr::filter(!grepl('Books', theme_name)) %>% 
     dplyr::filter(!grepl('Value', theme_name))  %>% 
     dplyr::filter(set_num != "SWMP-1") # StarWars Promo Mosaic set
     #dplyr::filter(n > 9) %>% dplyr::arrange(desc(n))
...
Here I subsetted those themes that have actual part-related sets., i.e., 
excluding themes like Books, Gear, etc.

I'm using the whole part list in order to estimate diversity parameters on sets.
