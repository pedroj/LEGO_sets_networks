...
```{r networks, echo=TRUE, fig.width=12}
# Input edgelist
...
model75344.g<- igraph::graph_from_edgelist(as.matrix(cbind(model75344[,c(1,2)])),
                                   directed= F)
# E(model75344.g)$weight<- model75344[,4]
...
```{r ggraph_plots, echo=TRUE, fig.width=12}
library(ggraph)
ggraph(model75344.g, 'igraph', algorithm = 'tree', circular = TRUE) + 
  geom_edge_diagonal(aes(alpha = after_stat(index))) +
  coord_fixed() + 
  scale_edge_alpha('Direction', guide = 'edge_direction') +
  geom_node_point(aes(filter = degree(model75344.g, mode = 'out') == 0), 
