...
```{r parse, echo=TRUE}
# 1) Read file
mpd_lines <- readLines(here::here("data/models/7131-1-Anakin-Podracer.mpd"))
...
## 5. Optional: safer filename scheme
 ...
 ggraph(g, layout = "manual", x = coords[,1], y = coords[,2]) +
   geom_edge_link(width = 0.1, alpha = 0.8) +
    ...
    geom_image(
        ...
        size = 0.035,                 # fixed, reasonable size
        inherit.aes = FALSE
    ) +
    geom_node_text(aes(label = name),
                   vjust = 3.25,
